...
void DLListBefore (DLList L, char *it)//TODO
{
	assert (L != NULL);
	...
	if(L->curr == L->first) {
	    ...
	    L->curr = new;
	//empty list
	}else if(L->curr == NULL && DLListLength(L) == 0) {
	    L->curr = new;
	    L->first = new;
	    L->last = new;
	//current in the middle of the list
	}else {
...
void DLListDelete (DLList L)
{
	...
	if(L->curr->next == NULL && DLListLength(L) > 1) {
	    L->curr->prev->next = NULL;
	    L->curr = L->curr->prev;
	    L->last = L->curr;
	} else if(L->curr->next == NULL && DLListLength(L) == 1) {
	    L->curr = NULL;
